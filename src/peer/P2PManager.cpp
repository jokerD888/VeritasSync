...
void P2PManager::set_multi_wan_config(bool enabled, std::string url) {
    m_multi_wan_enabled = enabled;
    m_stun_list_url = std::move(url);
    g_logger->info("[Config] 多 WAN 探测功能: {}, 列表源: {}", enabled ? "启用" : "禁用",
                   enabled ? m_stun_list_url : "N/A");
}

std::vector<std::string> P2PManager::fetch_stun_servers(const std::string& url) {
    std::vector<std::string> servers;
    if (url.empty()) return servers;

    try {
        // 简单的 URL 解析
        std::string proto = "https://";
        size_t proto_pos = url.find(proto);
        if (proto_pos != 0) {
            // 尝试 http
            proto = "http://";
            proto_pos = url.find(proto);
            if (proto_pos != 0) {
                g_logger->error("[MultiWAN] URL 必须以 http:// 或 https:// 开头");
                return servers;
            }
        }

        std::string url_no_proto = url.substr(proto.length());
        size_t path_pos = url_no_proto.find('/');
        std::string host = (path_pos == std::string::npos) ? url_no_proto : url_no_proto.substr(0, path_pos);
        std::string path = (path_pos == std::string::npos) ? "/" : url_no_proto.substr(path_pos);

        // 创建 Client (自动处理 HTTPS/HTTP)
        httplib::Client cli(proto + host);
        ...
        if (res && res->status == 200) {
            ...
            g_logger->warn("[MultiWAN] 下载 STUN 列表失败: Status {}", res ? res->status : -1);
        }
    } catch (const std::exception& e) {
        g_logger->error("[MultiWAN] 下载异常: {}", e.what());
    }
    return servers;
}

std::string P2PManager::perform_simple_stun_request(boost::asio::io_context& ioc, const std::string& host,
                                                    uint16_t port) {
    ...
    try {
        ...
        socket.send_to(boost::asio::buffer(req), *endpoints.begin());

        std::vector<uint8_t> resp(1024);
        udp::endpoint sender_ep;

        // 阻塞接收 (由 socket 选项控制超时)
        boost::system::error_code ec;
        size_t len = socket.receive_from(boost::asio::buffer(resp), sender_ep, 0, ec);
        ...
        while (pos + 4 <= len) {
            ...
            uint16_t length = (resp[pos + 2] << 8) | resp[pos + 3];

            if (pos + 4 + length > len) break;

            if (type == 0x0001 || type == 0x0020) {  // Mapped Address
                if (resp[pos + 5] == 0x01) {         // IPv4
                    uint32_t ip_int =
                        (resp[pos + 8] << 24) | (resp[pos + 9] << 16) | (resp[pos + 10] << 8) | resp[pos + 11];
                    if (type == 0x0020) ip_int ^= 0x2112A442;  // XOR 处理
                    return boost::asio::ip::address_v4(ip_int).to_string();
                }
            }
            pos += 4 + length;
        }
...
void P2PManager::start_multi_wan_probe() {
    ...
    boost::asio::post(m_worker_pool, [this, self = shared_from_this()]() {
        ...
        int new_ips = 0;
        {
            ...
            for (const auto& ip : temp_results) {
                if (m_detected_wan_ips.find(ip) == m_detected_wan_ips.end()) {
                    m_detected_wan_ips.insert(ip);
                    new_ips++;
                }
            }
        }
...
std::string P2PManager::generate_extra_candidates_sdp(const std::string& base_sdp) {
    std::lock_guard<std::mutex> lock(m_wan_ips_mutex);
    if (m_detected_wan_ips.empty()) return "";

    std::stringstream ss_in(base_sdp);
    std::string line;
    ...
    while (std::getline(ss_in, line)) {
        ...
        if (line.find("a=candidate:") == 0) {
            if (line.find("typ srflx") != std::string::npos) {
                candidate_template = line;
                break;
            }
            if (candidate_template.empty() && line.find("typ host") != std::string::npos) {
                candidate_template = line;
            }
        }
    }

    if (candidate_template.empty()) return "";

    // 2. 解析模板
    std::stringstream ss_parts(candidate_template);
    std::vector<std::string> parts;
    std::string part;
    while (ss_parts >> part) parts.push_back(part);
    if (parts.size() < 8) return "";

    std::string original_ip = parts[4];
    std::string extra_sdp;

    // 3. 生成新行
    for (const auto& wan_ip : m_detected_wan_ips) {
        if (wan_ip == original_ip) continue;

        std::stringstream new_line;
        for (size_t i = 0; i < parts.size(); ++i) {
            if (i == 0) {
                // 修改 Foundation ID (例如 "candidate:1" -> "candidate:19") 避免冲突
                new_line << parts[i] << "9";
            } else if (i == 3) {
                // 修改优先级 (Priority)，稍微降低
                try {
                    uint32_t prio = std::stoul(parts[i]);
                    new_line << (prio > 100 ? prio - 100 : prio);
                } catch (...) {
                    new_line << parts[i];
                }
            } else if (i == 4) {
            ...
            if (i < parts.size() - 1) new_line << " ";
        }
        extra_sdp += new_line.str() + "\r\n";
    }
...
void P2PManager::handle_juice_gathering_done(juice_agent_t* agent) {
    ...
    if (juice_get_local_description(agent, local_description, sizeof(local_description)) == 0) {
        ...
        std::string extra = generate_extra_candidates_sdp(sdp_str);
        if (!extra.empty()) {
            g_logger->info("[MultiWAN] 注入 {} 个额外 WAN IP 到 SDP。", m_detected_wan_ips.size());
            if (!sdp_str.empty() && sdp_str.back() != '\n') sdp_str += "\r\n";
            sdp_str += extra;
        }
